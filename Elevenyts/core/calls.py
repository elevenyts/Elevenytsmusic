                    ...
                    if ok:
                        ...
                        try:
                            media.message_id = await send_player(
                                target_chat_for_messages, text, _thumb, media, playing=True
                            )
                        except Exception as rich_error:
    ...
    async def play_next(self, chat_id: int, control_message=None, force_new_message: bool = False) -> None:
        ...
        async with lock:
            try:
                ...
                # Queue is empty: if Autoplay is enabled, automatically find
                # a different/similar track based on the song that just ended.
                # The new track is inserted as the current queue item so the
                # Rich Player and all controls continue to work normally.
                if not media:
                    autoplay_enabled = await db.get_autoplay(chat_id)
                    if autoplay_enabled and current:
                        try:
                            auto_track = await yt.search_related(
                                current.title,
                            ...
                            logger.warning(
                                f"Autoplay search failed for {chat_id}: {e}")
                            auto_track = None

                        if auto_track:
                            auto_track.user = getattr(current, "user", None) or "Autoplay"
                            queue.queues[chat_id].appendleft(auto_track)
                            media = auto_track
                            logger.info(
                                f"🎵 [AUTOPLAY] Starting related track for {chat_id}: "
                                f"{auto_track.title} ({auto_track.id})"
                            )
                        else:
                            logger.info(
                                f"🎵 [AUTOPLAY] No related track found for {chat_id}")
